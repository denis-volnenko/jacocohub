<#include '/include/header.ftl'>

<h1>СПРАВКА</h1>

<h2>СЕРВЕР</h2>

<table width="100%" cellspacing="0" cellpadding="10" border="1">
    <tr>
        <th nowrap="nowrap" align="left">groupId</th>
        <td width="100%" align="left">ru.volnenko.cloud</td>
    </tr>
    <tr>
        <th nowrap="nowrap" align="left">artifactId</th>
        <td width="100%" align="left">jacocohub</td>
    </tr>
    <tr>
        <th nowrap="nowrap" align="left">version</th>
        <td width="100%" align="left">1.0.0</td>
    </tr>
</table>

<h2>ПЛАГИН</h2>

<table width="100%" cellspacing="0" cellpadding="10" border="1">
    <tr>
        <th nowrap="nowrap" align="left">groupId</th>
        <td width="100%" align="left">ru.volnenko.plugin</td>
    </tr>
    <tr>
        <th nowrap="nowrap" align="left">artifactId</th>
        <td width="100%" align="left">jacocohub-maven-plugin</td>
    </tr>
    <tr>
        <th nowrap="nowrap" align="left">version</th>
        <td width="100%" align="left">1.0.0</td>
    </tr>
</table>

<h2>РАЗРАБОТЧИК</h2>

<table width="100%" cellspacing="0" cellpadding="10" border="1">
    <tr>
        <th nowrap="nowrap" align="left">Разработчик</th>
        <td width="100%" align="left">Денис Волненко</td>
    </tr>
    <tr>
        <th align="left">Веб-сайт</th>
        <td align="left">https://www.volnenko.ru</td>
    </tr>
    <tr>
        <th align="left">Почта</th>
        <td align="left">denis@volnenko.ru</td>
    </tr>
</table>

<#include '/include/footer.ftl'>